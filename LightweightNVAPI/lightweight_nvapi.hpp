...
namespace lightweight_nvapi {
	...
	using cb_NvAPI_D3D12_QueryCpuVisibleVidmem = nvstatus_t(*)(ID3D12Device* pDevice, uint64_t* pTotalBytes, uint64_t* pFreeBytes);

	constexpr nvintfid_t id_NvAPI_D3D12_UseDriverHeapPriorities = 0xF0D978A8;
	using cb_NvAPI_D3D12_UseDriverHeapPriorities = nvstatus_t(*)(ID3D12Device* pDevice);
	...
	using cb_NvAPI_D3D12_CreateCommittedResource = nvstatus_t(*)(ID3D12Device* pDevice,
		const D3D12_HEAP_PROPERTIES* pHeapProperties,
		D3D12_HEAP_FLAGS         HeapFlags,
		const D3D12_RESOURCE_DESC* pDesc,
		D3D12_RESOURCE_STATES    InitialState,
		const D3D12_CLEAR_VALUE* pOptimizedClearValue,
		const NV_RESOURCE_PARAMS* pNVResourceParams,
		REFIID                    riid,
		void** ppvResource,
		bool* pSupported);
	...
	bool nvapi_state_t::init_ptrs() {
		if (!nvapi64_)
			return false;
		queryinterface_ = reinterpret_cast<void* (*)(unsigned)>(GetProcAddress(nvapi64_, "nvapi_QueryInterface"));
